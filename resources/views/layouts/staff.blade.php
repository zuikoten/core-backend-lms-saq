...
<body class="bg-slate-50 antialiased" style="font-family: 'Plus Jakarta Sans', sans-serif;">
    <div class="flex min-h-screen" x-data="{ confirmingLogout: false }">
        ...
        <div x-show="confirmingLogout" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4"
            style="background: rgba(15, 23, 42, 0.4);" @keydown.escape.window="confirmingLogout = false">
            <div @click.outside="confirmingLogout = false" x-show="confirmingLogout" x-transition
                class="bg-white rounded-2xl p-6 w-full max-w-xs" style="box-shadow: 0 2px 10px rgba(20,20,50,0.06);">
                ...
                <div class="flex gap-2">
                    ...
                    <form method="POST" action="{{ route('staff.logout') }}" class="flex-1">
                        @csrf
                        <button type="submit"
                            class="w-full rounded-xl bg-red-500 text-white text-sm font-medium py-2 hover:bg-red-600 transition">
                            Ya, Keluar
                        </button>
                    </form>
                </div>
