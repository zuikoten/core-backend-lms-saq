...
@section('title', 'Tambah Pemetaan Tarif')

@section('content')
<div class="max-w-xl">
    <h1 class="mb-6 text-xl font-semibold text-slate-800">Tambah Pemetaan Tarif</h1>

    <form action="{{ route('finance.student-tariff-mappings.store') }}" method="POST" class="rounded-2xl bg-white p-6 shadow-sm">
        @csrf
        ...
        <div class="mb-4">
            ...
            <p class="mt-1 text-xs text-slate-400">Isi kalau tarif ini beda dari tarif standar (diskon/tarif khusus) dan sudah disahkan Kepala Sekolah.</p>
            @error('approved_by')
                <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea name="note" rows="3"
                      class="w-full rounded-xl border-slate-200 px-3.5 py-2.5 text-sm focus:border-indigo-400 focus:ring-indigo-400"
                      placeholder="Contoh: Diskon tidak mampu, tarif ABK, anak atlet berprestasi, dll. Wajib diisi kalau ada persetujuan.">{{ old('note') }}</textarea>
            @error('note')
                <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
            @enderror
        </div>
